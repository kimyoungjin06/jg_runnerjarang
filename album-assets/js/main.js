...
var main = (function($) { var _ = {
	...
	initProperties: function() {
		...
		_.$viewer = $(
			'<div id="viewer">' +
				'<div class="inner">' +
					...
					'<a class="download" href="#" download>사진 다운로드</a>' +
				'</div>' +
			'</div>'
		).appendTo(_.$body);
		...
		// Download.
			_.$download = _.$viewer.find('.download');
	...
	initEvents: function() {
		...
		// Download.
			_.$download.on('click', function(event) {
				event.stopPropagation();
			});

			_.$download.on('touchend', function(event) {
				event.stopPropagation();
			});
	...
	/**
	 * Updates download button for the current slide.
	...
	setDownload: function(url) {
		...
		_.$download.attr('href', url);

		if (filename)
			_.$download.attr('download', filename);
